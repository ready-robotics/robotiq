...
from robotiq_c_model_control.robotiq_service_interface import RobotiqServiceInterface
...
class RobotiqGripper(RobotiqServiceInterface):
    def __init__(self, teachmate_type):
        super(RobotiqGripper, self).__init__()
        if teachmate_type == MODBUS:
    ...
    def initialize_device(self, teachmate):
        """
        This function connections the Robotiq device. Depending on the teachmate selected it either connects to a port
        or tests the already established connection for connectivity.
        Args:
            teachmate: The type of teachmate, this is specified in the ready_launch config file for each robot
...
if __name__ == '__main__':
    ...
    teachmate = rospy.get_param('/robot_configuration/teachmate_type', 'original')
    gripper = RobotiqGripper(teachmate)
    if gripper.initialize(teachmate):
        gripper.start()
    rospy.spin()
